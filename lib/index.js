...
function listedHeader(item) {
    const record = item;
    if (record === undefined)
        return undefined;
    if (record.header !== undefined && record.header.id !== undefined)
 ...
 */
async function readStoredSession(sessionId, service) {
    ...
    if (typeof modern.open !== 'function')
        return undefined;
        ...
        try {
            ...
            if (presets !== undefined) {
                try {
                    const stored = await readStoredSession(sessionId, service);
                    if (stored !== undefined)
                        presetId = resolveSessionPresetOf(stored);
                }
                catch (error) {
